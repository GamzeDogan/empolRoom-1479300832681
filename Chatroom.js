...
io.on('connection', function(socket) {
	...
	socket.on('weatherAPI', function(msg){
		...
		if(message.match('atlanta') != undefined){
			...
			//var urlLocation = '';
			//var url;
			requestLocation('https://'+weather.username+':'+weather.password+'@twcservice.mybluemix.net:443/api/weather/v3/location/search?query=Atlanta&locationType=city&language=en-US', function(error, response){
			...
			request('https://'+weather.username+':'+weather.password+'@twcservice.mybluemix.net:443/api/weather/v1/geocode/45.42/75.69/forecast/daily/10day.json?units=m&language=en-US', function(error, response){
				if(response.statusCode >= 200 && response.statusCode < 400){
					var content = JSON.parse(response.body.metadata[0]);
